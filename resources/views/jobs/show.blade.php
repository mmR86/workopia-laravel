                    ...
                    <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
                        <div @click.away="open = false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                            ...
                            <form method="POST" action="{{route('applicant.store', $job->id)}}" enctype="multipart/form-data">
                                ...
                                <button @click="open = false" class="bg-gray-300 hover:bg-gray-200 text-blacl px-4 py-2 rounded-md">Cancel</button>
                            </form>
                        </div>
                    </div>
                </div> 
                @else 
                <p class="my-5 bg-gray-200 rounded-xl p-3 font-semibold">
                    <i class="fas fa-info-circle mr-3"></i> You must be logged in for apply for this job!
                </p>
                @endauth
            </div>
